...
public:
	...
	void displayInfo() const {
		cout << "Model: " << Model << ", Storage: " << Storage << " ГБ" << endl;
	}
};
...
class Jail {
	...
	string name;
public:
	Jail(int room, string name) : room(room), name(name){}
...
int main() {
	...
	int count = 0;
	vector<Jail> jail = { Jail(1, "jail1"), Jail(2, "jail2"), Jail(3, "jail3"), Jail(4, "jail4"), Jail(5, "jail5") };
	for_each(jail.begin(), jail.end(), [&count](Jail& elem) {count += elem.getRoom(); }); //лямнда функції
